...
function Logo() {
  return (
    <div className="brand">
      <img src="/images/Logo.jpeg" alt="RailOptix" className="logo-image" />
      <div>
...
function Sidebar({ open, onClose }) {
  ...
  const logout = () => {
    ...
    localStorage.removeItem('railoptix-token')
  ...
  return (
    <aside className={`sidebar ${open ? 'open' : ''}`}>
      <div className="sidebar-head">
        <Logo/>
        <button className="icon-btn close-mobile" onClick={onClose} aria-label="Close navigation"><X/></button>
      </div>
        ...
        {['OPERATIONS','ASSETS','PLANNING','INSIGHTS','SYSTEM'].map(group => (
          <div className="nav-group" key={group}>
            ...
            {nav.filter(n => n.group === group).map(({label,href,icon:Icon}) => (
              <Link key={label} href={href} className={`nav-item ${label === 'Dashboard' ? 'active' : ''}`} onClick={onClose}>
                <Icon/>
                <span>{label}</span>
                {label === 'Dashboard' && <i/>}
              </Link>
      ...
      <div className="sidebar-foot">
        ...
        {userName ? (
          ...
          <div style={{display:'flex',alignItems:'center',gap:'8px'}}>
            <Link href="/login" className="secondary-btn" style={{textDecoration:'none'}}>LOGIN</Link>
            <Link href="/signup" className="primary-btn" style={{textDecoration:'none'}}>SIGN UP</Link>
          </div>
        )}
      </div>
    </aside>
...
function Topbar({ onMenu, onNotify }) {
  ...
  return (
    <header className="topbar">
      <button className="icon-btn menu-btn" onClick={onMenu} aria-label="Open navigation"><Menu/></button>
      ...
      <div className="context">
        ...
        <button className="status" onClick={() => setShowZone(!showZone)}>
          <span className="pulse-dot"></span> OPERATIONAL
        </button>

        {showZone && (
          <div style={{position:'absolute',top:'52px',left:'18px',padding:'12px 14px',background:'var(--panel)',border:'1px solid var(--line)',zIndex:20,fontSize:'11px'}}>
            <strong>System Status</strong>
            <div style={{marginTop:'5px',color:'var(--muted)'}}>All services nominal</div>
          </div>
        )}
      </div>
      ...
      <div className="top-actions">
        <div className="sync">
          <span className="sync-dot"></span> DATA SYNCED <b style={{color:'var(--text)'}}>{time}</b>
        </div>

        <div className="date">
          {date} <b style={{color:'var(--text)'}}>{time ? `${time.slice(0,5)} IST` : ''}</b>
        </div>

        <button className="icon-btn notify" onClick={onNotify} aria-label="Notifications">
          <Bell/><em>3</em>
        </button>
...
function MetricStrip({ overview }) {
  const metrics = [
    ['Assets Monitored', overview?.summary?.totalAssets ?? '—', '', 'info', Database],
    ['High-Risk Assets', overview?.summary?.criticalAssets ?? '—', '', 'critical', AlertTriangle],
    ['Active Maintenance Tasks', overview?.summary?.activeMaintenanceTasks ?? '—', '', 'warn', Wrench],
    ['Today’s Block Requests', overview?.summary?.activeBlocks ?? '—', '', 'info', GitBranch],
    ['Network Availability', overview?.kpis?.infrastructureAvailability != null
      ? `${overview.kpis.infrastructureAvailability}%`
      : '—', '', 'up', Gauge],
    ['Average Delay Risk', overview?.kpis?.expectedTrainDelay != null
      ? `${overview.kpis.expectedTrainDelay} min`
      : '—', '', 'down', TrendingDown],
  ]

  return (
    <div className="metric-grid">
      {metrics.map(([label,value,trend,type,Icon]) => (
  ...
  return (
    <section className="panel twin">
      <div className="panel-head">
        <div>
          <div className="section-kicker"><Map/> LIVE NETWORK VIEW</div>
          <h2>Railway Digital Twin</h2>
          <p>Delhi Division · 248.6 route km monitored</p>
        </div>

        <div className="panel-tools">
          <button className="tool-btn active">LIVE</button>
          <button className={`tool-btn ${showFilters ? 'active' : ''}`} onClick={() => setShowFilters(!showFilters)}>
      ...
      {showFilters && (
        <div style={{display:'flex',gap:'6px',padding:'10px 16px',borderBottom:'1px solid var(--line)',flexWrap:'wrap'}}>
          {['ALL','HEALTHY','AT RISK','HIGH RISK','BLOCKS','TRAINS'].map(item => (
            <button key={item} className={`tool-btn ${filter === item ? 'active' : ''}`} onClick={() => setFilter(item)}>
              {item}
            </button>
          ))}
        </div>
      ...
      <div className="twin-map">
        ...
        <div className="map-readout">
          <span><i className="green-dot"></i> 1,221 HEALTHY</span>
          <span><i className="yellow-dot"></i> 12 BLOCKS</span>
          <span><i className="red-dot"></i> 27 HIGH RISK</span>
        </div>

        <div className="legend">
          <span><i className="legend-train"></i> TRAIN</span>
          <span><i className="legend-line"></i> TRACK</span>
          <span><i className="legend-signal"></i> SIGNAL</span>
          <span><i className="legend-ohe"></i> OHE</span>
          <span><i className="legend-block"></i> BLOCK</span>
        </div>
      </div>
    </section>
  )
  ...
  return (
    <section className="panel priority">
      <div className="panel-head compact">
        <div>
          <div className="section-kicker"><BrainCircuit/> AI PRIORITIZED</div>
          <h2>AI Maintenance Priority</h2>
        </div>
        <Link href="/maintenance" className="text-btn">VIEW ALL <span>→</span></Link>
      </div>
      ...
      {priorities.map(p => (
        <Link href={`/assets/${p.asset}`} className="priority-row" key={p.id}>
          <div className="asset-title">
            <b>{p.id}</b>
            <div><strong>{p.asset}</strong><small>{p.title}</small></div>
          </div>

          <div className="risk-cell">
            <strong className={p.color}>{p.risk}</strong>
            <div className="risk-bar"><i className={p.color} style={{width:p.risk}}></i></div>
            <small className={p.color}>{p.level}</small>
          </div>

          <span className="impact">{p.impact}</span>
          <span className="deadline">{p.due}</span>
        </Link>
      ))}
    </section>
  )
...
function Blocks() {
  ...
  return (
    <section className="panel blocks">
      <div className="panel-head compact">
        <div>
          <div className="section-kicker"><GitBranch/> DISPATCH SCHEDULE</div>
          <h2>Upcoming Blocks</h2>
        </div>
        <Link href="/blocks" className="icon-btn" aria-label="View blocks"><Plus/></Link>
      </div>
  ...
  return (
    <section className="recommendation">
      <div className="recommend-head">
        ...
        <div>
          <div className="section-kicker">AI RECOMMENDATION <span className="live-tag">LIVE</span></div>
          <h2>Consolidate planned work</h2>
        </div>
      ...
      {expanded && (
        <div style={{margin:'0 0 15px',padding:'12px 14px',border:'1px solid var(--line)',background:'var(--elevated)',color:'var(--muted)',fontSize:'12px',lineHeight:1.5}}>
          RailOptix recommends consolidation because both activities can be handled within the same operational window, reducing additional restrictions.
        </div>
      )}

      <div className="rec-stats">
        <div><strong>4</strong><span>CONFLICTS AVOIDED</span></div>
        <div><strong>23 min</strong><span>DELAY REDUCTION</span></div>
        <div><strong>+2.1%</strong><span>ASSET AVAILABILITY</span></div>
      </div>

      <div className="rec-actions">
        <Link href="/planning/weekly" className="primary-btn">REVIEW PLAN <span>→</span></Link>
        <button className="secondary-btn" onClick={() => setExpanded(!expanded)}>VIEW EXPLANATION</button>
      </div>
    </section>
  )
}
...
function Analytics() {
  ...
  return (
    <section className="panel analytics">
      <div className="panel-head compact">
        <div>
          <div className="section-kicker"><Activity/> SYSTEM TELEMETRY</div>
        ...
        <div style={{position:'relative'}}>
          ...
          {open && (
            <div style={{position:'absolute',right:0,top:'calc(100% + 6px)',zIndex:30,minWidth:'130px',background:'var(--panel)',border:'1px solid var(--line)',boxShadow:'0 8px 20px rgba(0,0,0,.06)'}}>
              {['24 HOURS','7 DAYS','30 DAYS'].map(item => (
                <button
                  key={item}
                  onClick={() => {setRange(item);setOpen(false)}}
                  style={{display:'block',width:'100%',padding:'9px 12px',border:0,background:item === range ? 'var(--elevated)' : 'transparent',color:'inherit',textAlign:'left',fontSize:'11px',cursor:'pointer'}}
                >
                  LAST {item}
                </button>
              ))}
...
function QuickActions() {
  return (
    <div className="quick-actions">
      ...
      <Link href="/maintenance/new" style={{border:'1px solid var(--line)',background:'#FFFFFF',padding:'10px 12px',color:'#53636A',fontSize:'10px',letterSpacing:'.04em',display:'flex',alignItems:'center',gap:'7px',borderRadius:'3px',textDecoration:'none'}}>
        <Plus/> CREATE MAINTENANCE TASK
      </Link>

      <Link href="/blocks" style={{border:'1px solid var(--line)',background:'#FFFFFF',padding:'10px 12px',color:'#53636A',fontSize:'10px',letterSpacing:'.04em',display:'flex',alignItems:'center',gap:'7px',borderRadius:'3px',textDecoration:'none'}}>
        <GitBranch/> REQUEST BLOCK
      </Link>

      <Link href="/simulation" style={{border:'1px solid var(--line)',background:'#FFFFFF',padding:'10px 12px',color:'#53636A',fontSize:'10px',letterSpacing:'.04em',display:'flex',alignItems:'center',gap:'7px',borderRadius:'3px',textDecoration:'none'}}>
        <Play/> RUN SIMULATION
      </Link>

      <Link href="/scheduler" style={{border:'1px solid #A7D7D0',background:'#E7F4F1',padding:'10px 12px',color:'#17665D',fontSize:'10px',letterSpacing:'.04em',display:'flex',alignItems:'center',gap:'7px',borderRadius:'3px',textDecoration:'none'}}>
        <Zap/> RUN OPTIMIZATION
      </Link>
    </div>
  )
}
...
export default function Page() {
  ...
  useEffect(() => {
    async function loadDashboard() {
      try {
        const data = await apiFetch('/api/dashboard/overview?corridorId=COR-001')
        setOverview(data)
      } catch (err) {
    ...
    <div className="app-shell">
      ...
      <div className="main-area">
        <Topbar
          ...
          onNotify={() => setNotifications(!notifications)}
        />

        {notifications && (
          <div style={{position:'fixed',top:'70px',right:'20px',zIndex:100,width:'280px',padding:'16px',background:'var(--panel)',border:'1px solid var(--line)',boxShadow:'0 10px 30px rgba(0,0,0,.08)'}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'12px'}}>
              <strong>Notifications</strong>
              <button className="icon-btn" onClick={() => setNotifications(false)} aria-label="Close notifications"><X/></button>
            </div>
        ...
        <main className="dashboard">
          <div className="page-intro">
            <div>
              <div className="breadcrumb">OPERATIONS <span>/</span> COMMAND CENTER</div>
              <h1>Dashboard</h1>
              <p>Predict. Optimize. Simulate. Decide.</p>
            </div>

            <div className="intro-actions">
              <Link href="/planning/weekly" className="secondary-btn">
                <CalendarClock/> {new Date().toLocaleDateString('en-GB',{day:'2-digit',month:'short',year:'numeric'}).toUpperCase()}
              </Link>

              <Link href="/scheduler" className="primary-btn">
                <Zap/> OPTIMIZATION READY
              </Link>
            </div>
          </div>
        ...
        <footer>
          RAILOPTIX CONTROL SYSTEM <span>v2.4.1</span>
          <span className="footer-right">LAST SYSTEM CHECK 14:31:52 IST · NORTHERN RAILWAY</span>
        </footer>
      </div>
    </div>
  )
