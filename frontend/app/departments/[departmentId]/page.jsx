  ...
  return (
    <main className="dashboard">
      ...
      <div className="metric-grid" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(165px, 1fr))' }}>
        {metrics.map(([label, value, detail]) => <div className="metric" key={label}><span>{label}</span><strong>{value}</strong><small>{detail}</small></div>)}
      </div>
      ...
      <div className="main-grid">
        <section className="panel" style={{ overflow: 'hidden' }}>
          ...
          <div style={{ overflowX: 'auto' }}>
            <div className="table-head" style={{ minWidth: '650px', display: 'grid', gridTemplateColumns: '1.6fr .7fr .7fr .7fr 1fr', gap: '12px' }}>
            ...
            <div style={{ minWidth: '650px' }}>
              {departments.map((department) => {
                ...
                return <button key={department.id} onClick={() => setSelectedDepartment(department)} aria-pressed={isSelected} style={{ width: '100%', display: 'grid', gridTemplateColumns: '1.6fr .7fr .7fr .7fr 1fr', gap: '12px', alignItems: 'center', padding: '13px 16px', border: 0, borderBottom: '1px solid var(--line)', borderLeft: isSelected ? '3px solid var(--teal)' : '3px solid transparent', background: isSelected ? '#E7F4F1' : 'var(--panel)', textAlign: 'left', minHeight: '62px', color: 'var(--foreground)', fontFamily: 'inherit', cursor: 'pointer' }}>
                  ...
                  <span className={`block-state ${department.state === 'warning' ? 'soon' : ''}`} style={{ color: department.state === 'healthy' ? 'var(--green)' : 'var(--yellow)' }}>{department.status}</span>
                </button>
              })}
            </div>
          </div>
        ...
        <aside className="panel">
          <div className="panel-head compact">
            <div><div className="section-kicker">SELECTED DEPARTMENT</div><h2>Department Overview</h2></div>
            <span className={`block-state ${selectedDepartment.state === 'warning' ? 'soon' : ''}`} style={{ color: selectedDepartment.state === 'healthy' ? 'var(--green)' : 'var(--yellow)' }}>{selectedDepartment.status}</span>
          </div>

          <div style={{ padding: '2px 18px 18px' }}>
            <h3 style={{ margin: '12px 0 4px', fontSize: '17px', fontWeight: 600 }}>{selectedDepartment.name}</h3>
            ...
            ].map(([label, value]) => <div key={label} className="table-head" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', padding: '10px 0' }}><span>{label}</span><span style={{ textAlign: 'right', fontSize: '12px', color: label === 'AVAILABILITY' ? 'var(--teal)' : undefined }}>{value}</span></div>)}

            <Link href={`/departments/${selectedDepartment.id}`} className="secondary-btn" style={{ width: '100%', justifyContent: 'center', marginTop: '16px' }}>VIEW DEPARTMENT <ArrowRight /></Link>
          </div>
        </aside>
      </div>
      ...
      <section className="panel" style={{ overflow: 'hidden' }}>
        ...
        <div style={{ overflowX: 'auto' }}>
          <div className="table-head" style={{ minWidth: '720px', display: 'grid', gridTemplateColumns: '1.5fr 1fr .7fr .8fr 1fr', gap: '12px' }}><span>DEPARTMENT</span><span>WORKLOAD</span><span>BLOCKS</span><span>HOURS</span><span>AVAILABILITY</span></div>
          <div style={{ minWidth: '720px' }}>
            {departments.map((department) => <div key={department.id} style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr .7fr .8fr 1fr', gap: '12px', alignItems: 'center', padding: '13px 16px', minHeight: '62px', borderBottom: '1px solid var(--line)' }}>
